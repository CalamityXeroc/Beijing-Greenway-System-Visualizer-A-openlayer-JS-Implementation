...
本项目使用环境变量管理所有敏感信息（API密钥、数据库密码等）。请勿在代码中提交真实的凭证信息。详见 [配置文件](#-配置文件) 部分。
...
### 🌙 黑夜模式
- **全站深色主题**：包括主界面和所有14条绿道详情页
- **自动时间切换**：根据北京时间自动切换（6:00-18:00 为日间模式）
- **手动切换**：左上角主题按钮，支持一键切换
- **完整样式适配**：文本、背景、卡片、边框全部适配
- **localStorage 记忆**：记住用户的主题偏好设置
