...
### 🛠️ 专业 GIS 工具
- **绘制工具**：在地图上直接创建点、线、多边形，支持 **RGB 自定义颜色**
...
### 地图交互工具
- **绘制工具**：在地图上创建点、线、多边形，支持 **RGB 自定义颜色**
- **测量工具**：计算距离（米/公里）和面积（平方米/平方公里）
- **地图导出**：将当前地图视图导出为 PNG 文件
- **图层控制**：动态显示/隐藏地图图层
...
- **深色模式**：`[data-theme="night"]` 全站夜间主题支持
- **管理员状态显示**：登录管理员后，主页和地图页导航栏均显示管理员身份
- **LocalStorage 会话**：Auth Token 持久存储在 `localStorage`，页面刷新不丢失登录状态
