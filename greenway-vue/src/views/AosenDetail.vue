...
const loadGreenwayData = async () => {
  try {
    const response = await fetch('http://localhost:3000/api/greenways?name=奥林匹克森林公园绿道')
    if (!response.ok) {
      console.error('[AosenDetail] API请求失败:', response.status)
      return
    }
    const geojson = await response.json()
    console.log('[AosenDetail] 绿道数据加载成功:', geojson)
