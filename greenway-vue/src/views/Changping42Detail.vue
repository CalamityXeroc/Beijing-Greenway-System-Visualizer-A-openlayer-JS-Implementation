...
const loadGreenwayData = async () => {
  try {
    const response = await fetch('http://localhost:3000/api/greenways?name=昌平42绿道')
    if (!response.ok) {
      console.error('[Changping42Detail] API请求失败:', response.status)
      return
    }
    const geojson = await response.json()
    console.log('[Changping42Detail] 绿道数据加载成功:', geojson)
