...
const loadGreenwayData = async () => {
  try {
    const response = await fetch('http://localhost:3000/api/greenways?name=丽都商圈绿道')
    if (!response.ok) {
      console.error('[LiduDetail] API请求失败:', response.status)
      return
    }
    const geojson = await response.json()
    console.log('[LiduDetail] 绿道数据加载成功:', geojson)
