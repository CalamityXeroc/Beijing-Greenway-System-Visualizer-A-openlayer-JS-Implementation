...
const loadGreenwayData = async () => {
  try {
    const response = await fetch('http://localhost:3000/api/greenways?name=营城建都绿道')
    if (!response.ok) {
      console.error('[YingchengDetail] API请求失败:', response.status)
      return
    }
    const geojson = await response.json()
    console.log('[YingchengDetail] 绿道数据加载成功:', geojson)
